...
from models import Store, BusinessHours, TimeZone
...
def import_data():
    def parse_and_localize_timestamp(date_string):
    ...
    for i,stores_df in enumerate(stores_csv):
        print(f"***{i}***")
        stores_df = stores_df.dropna(subset=["timestamp_utc"])
        stores_df["timestamp_utc"] = pd.to_datetime(stores_df["timestamp_utc"], format="mixed").dt.tz_convert('UTC')
        for i, row in stores_df.iterrows():
            ...
            if (i + 1) % batch_size == 0:
                db.commit()
        db.commit()
    print(f"Number of stores: {db.query(Store).count()}")
...
# run iport_data() in a separate thread
def run_import_data():
    import_data()
...
def get_store_uptime_downtime(store_id, start_date, end_date):
    ...
    for day_offset in range((end_date - start_date).days + 1):
        ...
        local_start_time = datetime.combine(
            date,
            BusinessHours.query.filter_by(store_id=store_id, day_of_Week=day_of_week)
            .first()
            .start_time_local,
        )
